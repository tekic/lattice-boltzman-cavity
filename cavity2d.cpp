                  ...
                  OnLatticeBoundaryCondition2D<T,DESCRIPTOR>& bc )
{
	...
	for (int iX=1; iX<nx-1; ++iX) {
        T u = converter.getLatticeU();
        T vel[] = { u, T() };
        lattice.get(iX,0).defineRhoU((T)1, vel);
        lattice.get(iX,0).iniEquilibrium((T)1, vel);
    }*/
	for (int iY=1; iY<ny-1; ++iY) {
        T u = converter.getLatticeU();
        T vel[] = { T(), u  };
        lattice.get(0,iY).defineRhoU((T)1, vel);
        lattice.get(0,iY).iniEquilibrium((T)1, vel);
    }
	for (int iY=1; iY<ny-1; ++iY) {
        T u = converter.getLatticeU();
        T vel[] = { T(), u  };
        lattice.get(nx-1,iY).defineRhoU((T)1, vel);
        lattice.get(nx-1,iY).iniEquilibrium((T)1, vel);
    }
...
int main(int argc, char* argv[]) {
    ...
    for (iT=0; iT*converter.getDeltaT()<maxT; ++iT) {
		...
		if (iT%converter.nStep(vtkSave)==0 && iT>0) {
            cout << "Saving Velocity..." << endl;
            saveAsciiData(lattice.getDataAnalysis().getVelocity(),createFileName("velocity",iT,6));
        }
